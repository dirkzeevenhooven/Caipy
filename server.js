...
// ─── Frontend end-call trigger — returns immediately, webhook generates itinerary ─
app.post('/end-tavus-call', async (req, res) => {
  const { email, name, conversation_id } = req.body || {};
  if (!email) return res.json({ ok: false });

  console.log(`End call triggered for ${email} (conv: ${conversation_id}) — waiting for transcript webhook`);
  // Store lead so webhook can find the email when transcription_ready fires
  if (conversation_id && email) {
    tavusConversations.set(conversation_id, { email, name: name || '', createdAt: Date.now() });
  }
  // Return immediately — the real itinerary will be generated by the transcription_ready webhook
  res.json({ ok: true, pending: true, customerName: name || 'Traveller' });
});
